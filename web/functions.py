...
# ✅ 초기화 대기시간 줄임 (너무 길면 웹소켓 연결 타이밍 꼬임)
time.sleep(5)
...
class Functions(threading.Thread):
    ...
    def trackLineProcessing(self):
        ...
        앞바퀴 좌우 흔들기 / 앞으로-뒤로 살짝 움직이기
        """
        print("=== Touch Reaction Mode (trackLine 버튼) 활성화 ===")

        hand_detected = False  # 중복 방지용
        ...
        while True:
            ...
            # 손이 가까이 왔을 때 (15cm 미만)
            if dist < 15 and not hand_detected:
                hand_detected = True
                ...
                try:
                    if 'event_queue' in globals() and event_queue:
                        ...
                        print("[TOUCH] move 동작 완료 → 서버 전송 요청 (큐에 추가됨)")
                except Exception as e:
                    print(f"[ERROR] TOUCH 이벤트 큐 추가 실패: {e}")

            elif dist >= 20 and hand_detected:
                hand_detected = False
                print("[TOUCH] 손이 멀어짐 → 다시 감지 가능")
    ...
    def keepDisProcessing(self):
        ...
        print('keepDistanceProcessing:', distanceGet)
        if distanceGet > 40:
            move.move(60, 1, "mid")
        elif distanceGet < 30:
            move.move(60, -1, "mid")
...
# ✅ 전역 객체를 미리 만들어두고 외부에서 import 시 재사용
fuc = Functions()
fuc.setup()
fuc.start()
